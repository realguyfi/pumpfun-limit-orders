...
export async function buyCommand(token: string, amount: string, price: string, options: any) {
  const spinner = ora('Processing buy order...').start();
  ...
  try {
    ...
    spinner.succeed(chalk.green('Buy order created successfully!'));
    ...
    console.log(chalk.gray('\n💡 Your order will execute when the price drops to $' + numPrice.toFixed(6)));
    
  } catch (error: any) {
    spinner.fail(chalk.red('Failed to create buy order'));
